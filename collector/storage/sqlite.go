...
type IssueFilter struct {
	...
	Page     int
	PageSize int
	SortBy   string // last_seen, event_count, priority
}
...
func (db *DB) UpdateIssue(id int64, updates map[string]interface{}) error {
	...
	query := "UPDATE issues SET " + strings.Join(setParts, ", ") + " WHERE id = ?"
...
func (db *DB) GetSessionStats(appID string, startTime, endTime int64) (map[string]interface{}, error) {
	...
	func (db *DB) UpdateProject(id int64, updates map[string]interface{}) error {
		...
		query := "UPDATE projects SET " + strings.Join(setParts, ", ") + " WHERE id = ? AND deleted_at = 0"
		_, err := db.conn.Exec(query, args...)
		if err != nil {
