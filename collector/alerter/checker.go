...
func (c *Checker) checkRule(rule storage.AlertRule) (bool, string) {
	...
	default:
		slog.Error("Unknown condition type", "type", rule.ConditionType)
		return false, ""
	}
}
...
func (c *Checker) checkThreshold(rule storage.AlertRule) (bool, string) {
	...
	if err := json.Unmarshal([]byte(rule.ConditionConfig), &config); err != nil {
		slog.Error("Failed to parse threshold config", "error", err)
		return false, ""
	}
...
func (c *Checker) checkRate(rule storage.AlertRule) (bool, string) {
	...
	if err := json.Unmarshal([]byte(rule.ConditionConfig), &config); err != nil {
		slog.Error("Failed to parse rate config", "error", err)
		return false, ""
	}
	...
	if err != nil {
		slog.Error("Failed to query total events", "error", err)
		return false, ""
	}
	...
	if err != nil {
		slog.Error("Failed to query error events", "error", err)
		return false, ""
	}
...
func (c *Checker) triggerAlert(rule storage.AlertRule, message string) {
	slog.Error("Alert triggered", "rule", rule.Name, "message", message)
